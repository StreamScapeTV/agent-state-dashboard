------formdata-undici-001977140394
Content-Disposition: form-data; name="metadata"

{"main_module":"functionsWorker-0.9380428566332386.js"}
------formdata-undici-001977140394
Content-Disposition: form-data; name="functionsWorker-0.9380428566332386.js"; filename="functionsWorker-0.9380428566332386.js"
...
var fetchWithAuth = /* @__PURE__ */ __name((supabaseKey, supabaseUrl, getAccessToken, customFetch2, tracePropagationOptions, options) => {
...
var SUPABASE_URL = "https://fvbaxyklaclgdzyhybbr.supabase.co";
...
__name(isConfiguredProject, "isConfiguredProject");
function createAgentStateClient(env) {
...
__name(createAgentStateClient, "createAgentStateClient");

// ../pages-server/access.js
...
async function verifyCloudflareAccess(headers, env) {
...
// ../pages-server/agent-state.js
var ACTOR_BATCH_SIZE = 28;
var ALL_IDENTITIES = [
  "Orchestrator",
...
var ACTOR_BATCH_COUNT = Math.ceil(ALL_IDENTITIES.length / ACTOR_BATCH_SIZE);
var AgentStateReadError = class extends Error {
  static {
    __name(this, "AgentStateReadError");
  }
  constructor(message2) {
    super(message2);
...
function toJson2(value) {
  ...
  if (Array.isArray(value)) return value.map(toJson2);
  if (isRecord(value)) {
    return Object.fromEntries(Object.entries(value).map(([key, child]) => [key, toJson2(child)]));
  }
  return String(value);
}
__name(toJson2, "toJson");
function toJsonArray(value) {
  return Array.isArray(value) ? value.map(toJson2) : [];
}
__name(toJsonArray, "toJsonArray");
...
__name(normalizeActor, "normalizeActor");
async function readOverview(env, project) {
  ...
  return {
    project,
    projectState: toJson2(projectResult.data ?? {}),
    storageBudget: toJson2(storageResult.data ?? {}),
    actorBatchCount: ACTOR_BATCH_COUNT,
    actorCapacity: ALL_IDENTITIES.length,
    scannedAt: (/* @__PURE__ */ new Date()).toISOString()
  };
}
__name(readOverview, "readOverview");
async function readActorBatch(env, project, batch) {
...
__name(readActorBatch, "readActorBatch");
...
async function onRequestGet(context) {
  ...
  try {
    ...
    const message2 = error instanceof AgentStateReadError ? error.message : "Agent State read failed.";
...
async function onRequestGet2(context) {
  const access = await verifyCloudflareAccess(context.request.headers, context.env);
  if (!access.ok) return json2({ error: access.reason }, 403);
  const url = new URL(context.request.url);
  const project = url.searchParams.get("project") ?? "";
  if (!isConfiguredProject(project)) return json2({ error: "Unknown project" }, 404);
  try {
    return json2({ ...await readOverview(context.env, project), viewer: access.viewer });
  } catch (error) {
    const message2 = error instanceof AgentStateReadError ? error.message : "Agent State read failed.";
    return json2({ error: message2 }, 502);
  }
}
__name(onRequestGet2, "onRequestGet");

// ../.wrangler/tmp/pages-DFbWNj/functionsRoutes-0.019594116167394038.mjs
var routes = [
  {
    routePath: "/api/actors",
    mountPath: "/api",
    method: "GET",
    middlewares: [],
